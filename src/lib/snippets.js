...
export function createAutocompletion(additional){
  return (context)=>{
    ...
    let options=[];
    if(inFunction){
      options=snippets.inFunction;
      options=options.concat(additional);
    }else{
      options=snippets.topLevel;
...
function createSnippets(data){
  ...
  for(let ev in data.functions){
    ...
    snippets.inFunction.push(autocomplete.snippetCompletion(ev.name+createParamsString(ev.args,true), {
  ...
  for(let o in data.objects){
    ...
    for(let m in o.members){
      ...
      snippets.inFunction.push(autocomplete.snippetCompletion(o.name+"."+name+params, {
        label: o.name+"."+m.name,
  ...
  snippets.inFunction.push(autocomplete.snippetCompletion("Math.round(${x})", {
    label: "Math.round",
    info: "Rundet die Zahl auf Ganze.",
    type: "function"
  }));

  snippets.inFunction.push(autocomplete.snippetCompletion("Math.floor(${x})", {
    label: "Math.floor",
    info: "Rundet die Zahl auf Ganze ab.",
    type: "function"
  }));

  snippets.inFunction.push(autocomplete.snippetCompletion("Math.ceil(${x})", {
    label: "Math.ceil",
    info: "Rundet die Zahl auf Ganze auf.",
    type: "function"
  }));

  snippets.inFunction.push(autocomplete.snippetCompletion("Math.sin(${x})", {
    label: "Math.sin",
    info: "Berechnet den Sinus der Zahl im Bogenmaß.",
    type: "function"
  }));

  snippets.inFunction.push(autocomplete.snippetCompletion("Math.cos(${x})", {
    label: "Math.cos",
    info: "Berechnet den Kosinus der Zahl im Bogenmaß.",
    type: "function"
  }));

  snippets.inFunction.push(autocomplete.snippetCompletion("Math.sqrt(${x})", {
    label: "Math.sqrt",
    info: "Berechnet die Wurzel aus der Zahl.",
    type: "function"
  }));

  snippets.inFunction.push(autocomplete.snippetCompletion("Math.pow(${basis},${exp})", {
    label: "Math.pow",
  ...
  snippets.inFunction.push(autocomplete.snippetCompletion("for (let ${i} = 0; ${i} < ${max}; ${i}++) {\n\t${}\n}", {
      label: "for",
      info: "Eine for-Schleife wiederholt ihren Inhalt mehrere Male.",
      type: "keyword"
  }));

  snippets.inFunction.push(autocomplete.snippetCompletion("if (${bedingung}) {\n\t${}\n}", {
    label: "if",
